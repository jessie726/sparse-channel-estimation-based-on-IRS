...
clc;
rng(15000);
SNR_dB1 = 0:4:20;            %SNR测量点
SNR_length = length(SNR_dB1);%SNR长度
ite = 10;                  %迭代次数
...
NMSE_0 = zeros(SNR_length,1);
NMSE_1 = zeros(SNR_length,1);
NMSE_2 = zeros(SNR_length,1);
NMSE_3 = zeros(SNR_length,1);
...
for iter = 1 : ite
    for q = 1:Q   %----------------Q个时隙------------------
        ...
        noisepower  = P *10.^(-SNR_dB1(SNR_index)/10);                 %噪声功率
       ...
       %----------------信道模型----------------
        ...
        NMSE_0(SNR_index) = NMSE_0(SNR_index) + sum(sum(sum(abs(est_error).^2)));
         %-----------------LAOMP算法------------
        ...
        NMSE_1(SNR_index) = NMSE_1(SNR_index) + sum(sum(sum(abs(est_error1).^2)));
        ...
        NMSE_2(SNR_index) = NMSE_2(SNR_index) + sum(sum(sum(abs(est_error2).^2)));
        %-----------------SP算法-----------------
        ...
        NMSE_3(SNR_index) = NMSE_3(SNR_index) + sum(sum(sum(abs(est_error3).^2)));
end
end
NMSE_0 = 10*log10((NMSE_0) / ite /10/sum(sum(sum(abs(H_k_sparse).^2))));
NMSE_1 = 10*log10((NMSE_1)/ ite /10/sum(sum(sum(abs(H_k_sparse).^2))));
NMSE_2 = 10*log10((NMSE_2) / ite /10/sum(sum(sum(abs(H_k_sparse).^2))));
NMSE_3 = 10*log10((NMSE_3) / ite/10 /sum(sum(sum(abs(H_k_sparse).^2))));
save testone1.mat